...
T_inf = 245; -- Initial Temperature 
P_inf = 450; -- Initial Pressure 
rho_inf = 3*10-3 -- Initial Density 
mass_fraction = {N2=0.767, O2=0.233}; -- Mass Fractions of particles 

Q = GasState:new{gm} -- Gas Model 
Q.T = T_inf; 
Q.p = P_inf;
Q.rho = rho_inf;
...
inflow = FlowState:new{ p = P_inf , T= T_inf, velx=4363} -- Flow State 

-- Demo: Verify Mach number of inflow and compute dynamic pressure.
print("pressure=", inflow.p)
print("T=", inflow.T, "altitude=", 37000, "sound speed= ", inflow.a, "Pressure=", inflow.p)
...
blk0 = FluidBlock:new{grid=grid0, initialState=initial} -- Initial State 
blk1 = FluidBlock:new{grid=grid1, initialState=initial} -- Initial State 
blk2 = FluidBlock:new{grid=grid2, initialState=initial} -- Initial State 

-- Set boundary conditions.
identifyBlockConnections()
blk0.bcList['west'] = InFlowBC_Supersonic:new{flowState=inflow}  --Inflow from west edge
blk1.bcList['west']= InFlowBC_Supersonic:new{flowState=inflow}   --Inflow from west edge
blk0.bcList['east'] = WallBC_NoSlip_FixedT:new{Twall=3000}	-- Fixed wall temperature with no slip
blk1.bcList['east'] = WallBC_NoSlip_FixedT:new{Twall=3000}	-- Fixed wall temperature with no slip
blk2.bcList['east'] = WallBC_NoSlip_FixedT:new{Twall=3000}	-- Fixed wall temperature with no slip
blk1.bcList['west']=OutFlowBC_Simple:new{}	-- Outflow from the west edge
blk2.bcList['north']= OutFlowBC_Simple:new{}	-- Outflow from the north edge
blk2.bcList['west']= OutFlowBC_Simple:new{}	-- Outflow from the edge
...
config.fixed_time_step=false  -- No fixed time steps 
config.max_time = 6e-4  -- seconds
config.max_step = 1000000 -- Max Time 
config.dt_init = 1.0e-8  -- Time steps 
config.cfl_value = 0.5 
config.dt_plot = 2.5e-5 -- Plot evey 2.5e-5 seconds
config.dt_history = 1.0e-6  -- History data points steps
